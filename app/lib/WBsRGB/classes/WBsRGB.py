...
class WBsRGB:
    def __init__(self, gamut_mapping=2, upgraded=0):
        if upgraded == 1:
            self.features = np.load(
                f"{root_path}/../models/features+.npy"
            )  # encoded features
            self.mappingFuncs = np.load(
                f"{root_path}/../models/mappingFuncs+.npy"
            )  # correct funcs
            self.encoderWeights = np.load(
                f"{root_path}/../models/encoderWeights+.npy"
            )  # PCA matrix
            self.encoderBias = np.load(
                f"{root_path}/../models/encoderBias+.npy"
            )  # PCA bias
    ...
    def encode(self, hist):
        ...
        histR_reshaped = np.reshape(
            np.transpose(hist[:, :, 0]), (1, int(hist.size / 3)), order="F"
        )
        histG_reshaped = np.reshape(
            np.transpose(hist[:, :, 1]), (1, int(hist.size / 3)), order="F"
        )
        histB_reshaped = np.reshape(
            np.transpose(hist[:, :, 2]), (1, int(hist.size / 3)), order="F"
        )
        hist_reshaped = np.append(histR_reshaped, [histG_reshaped, histB_reshaped])
        feature = np.dot(
            hist_reshaped - self.encoderBias.transpose(), self.encoderWeights
        )
        return feature
    ...
    def correctImage(self, I):
        ...
        weightsH = np.exp(
            -(np.power(dH, 2)) / (2 * np.power(self.sigma, 2))
        )  # compute weights
        weightsH = weightsH / sum(weightsH)  # normalize blending weights
        mf = sum(
            np.matlib.repmat(weightsH, 1, 33) * mappingFuncs, 0
...
def normScaling(I, I_corr):
    ...
    I_corr[inds, :] = (
        I_corr[inds, :]
        / np.tile(norm_I_corr[:, np.newaxis], 3)
